...
class World {
  constructor(container, xrEnabledStops) {
    ...
    const coordinateList = xrEnabledStops.map((stop) => ({
      lat: stop.lat,
      lng: stop.lng,
    }));
    const locationList = xrEnabledStops.map((stop) => stop.name);
    const desc_list = xrEnabledStops.map((stop) => stop.description);

    pl_instance = new PanoramaLoader(coordinateList, locationList, desc_list);
    scene.add(camera, light);
    let promise = pl_instance.loadPanorama().then(
      (resolve) => {
        panorama = resolve.panorama_new;
        scene.add(panorama);
        let description = "\n" + resolve.description;
        panel = createPanel(resolve.locationName, description);
        scene.add(panel);
      },
      (reject) => {
        console.log(reject);
    ...
    document.getElementById("next-button").onclick = onSelectStart1;
    document.getElementById("prev-button").onclick = onSelectStart2;
...
function onSelectStart2(e) {
  ...
  let promise = pl_instance.movePanorama("prev").then(
    (resolve) => {
      console.log("prev");
...
function onSelectStart1(e) {
  ...
  let promise = pl_instance.movePanorama("next").then(
    (resolve) => {
      console.log("next");
